...
void BackTrack::OnCreateMove(CUserCmd* cmd)
{
	...
	for (int i = 1; i < g_EngineClient->GetMaxClients(); i++)
	{
		...
		float FOVDistance = Math::Distance_Point_line(hitboxPos, g_LocalPlayer->GetEyePos(), ViewDir);
	...
	if (bestTargetIndex != -1)
	{
		...
		{
			float tempFOVDistance = Math::Distance_Point_line(TimeWarpData[bestTargetIndex][t].hitboxPos, g_LocalPlayer->GetEyePos(), ViewDir);
